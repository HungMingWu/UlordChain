...
void RPCConsole::setClientModel(ClientModel *model)
{
    ...
    if (model && clientModel->getPeerTableModel() && clientModel->getBanTableModel()) {
        ...
        ui->banlistWidget->setColumnWidth(BanTableModel::Address, BANSUULORD_COLUMN_WIDTH);
...
void RPCConsole::clear()
{
    ...
    message(CMD_REPLY, (tr("Welcome to the Ulord Core RPC console.") + "<br>" +
...
void RPCConsole::unbanSelectedNode()
{
    ...
    CSubNet possibleSuulord(strNode.toStdString());

    if (possibleSuulord.IsValid())
    {
        CNode::Unban(possibleSuulord);
        DumpBanlist();
        clientModel->getBanTableModel()->refresh();
    }
}
